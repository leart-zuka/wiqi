    ...
    try {
      const response = await fetch("/api/getBlogPosts", {
        ...
        body: JSON.stringify({
          ...
          folder: ["quantum_tuesdays", "entries", "fraunhofer_isi"],
        }),
      });
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden space-x-8 xl:flex">
            ...
            <Link
              href={`/${props.locale}/posts/fraunhofer_isi`}
              className="group relative text-sm font-medium text-slate-700 transition-colors hover:text-purple-600 dark:text-slate-300 dark:hover:text-purple-400"
            >
              <p>Fraunhofer ISI</p>
              <span className="absolute bottom-0 left-0 h-0.5 w-0 bg-gradient-to-r from-purple-600 to-pink-500 transition-all duration-300 group-hover:w-full"></span>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="ml-2 rounded-full text-slate-700 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 xl:hidden"
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="border-t border-slate-200 py-4 dark:border-slate-700 xl:hidden">
            {/* Mobile Search */}
