...
The image below illustrates this superposition, where the cat’s state is represented as a quantum state vector. It combines the two equally probable \( (\frac{1}{\sqrt{2}}) \) possibilities of the cat being alive or dead. Opening the box will collapse the system into only one of the two states. The following equation presents a superposition, to which the Hadamard gate is applied:
