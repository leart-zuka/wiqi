...
Here, $ a_p^\dagger $ and $ a_q $ are the fermionic creation and annihilation operators, and the coefficients $ h_{pq} $ and $ h_{pqrs} $ correspond to one- and two-electron integrals that depend on the molecular orbitals. This Hamiltonian encodes all the interactions within the molecule and is the object that quantum algorithms like QPEA or the Variational Quantum Eigensolver (VQE) attempt to solve.
...
The typical workflow of such algorithms starts by mapping the Hamiltonian to qubit states and operations through the encoding of the fermionic states and operators. Fermions are a group of subatomic particles, including electrons or protons. They are a fundamental particles with a half-integer spin (1/2), that obey the Pauli exclusion principle. This principle states, that in a quantum system that obeys the laws of quantum mechanics, such as an atom, two particles with half-integer spins, such as fermions, can't simultaneously occupy the same quantum state. In an atom, two electrons always occupy the same quantum state at the same time. This means, that only one of the electrons can have a spin up (+ 1/2), and the other one has to have a spin down (-1/2).
