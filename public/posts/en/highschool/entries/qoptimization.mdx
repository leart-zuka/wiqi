...
_Image Source: [The Vessel Project](https://www.vesselproject.io/essays/life-through-quantum-annealing)_
...
Another exciting way to solve these problems is with Variational Quantum Algorithms (VQAs), where we create a team between a classical computer and a quantum computer.

We start of with an initial guess to solve the problem at hand which we then ask our quantum computer to evaluate. After receiving its side of the result, it gets handed to our classical computer to figure out wether our initial guess gave us the correct results. If that's the case then we can all be happy and go home. However, more often than not, if that's not the case, our classical computer will give us a new set of parameters based off of the result of the first experimental run, and we repeat the cycle until we receive our desired result.

The idea is to use a classical computer to tune the parameters of a quantum system. This allows us to find solutions by starting with a good guess and then refining it over and over. Sort of like making a pot out of clay.

## Visualizing the Quantum Advantage

A classical computer is like a hiker who walks downhill and stops in the first valley they find. They get stuck there, even if a much deeper valley is just over the next hill. While on the other hand, a quantum computer can use [_tunneling_](/en/posts/entries/highschool/qtunnelling) to pass straight through the hills, making it much easier to find the absolute lowest valley.

## Applications

This problem solving ability of Quantum Optimisation has impacted many fields where problems are too complex for the almost brute-force like nature of classical algorithms approaches. Finding e.g. the shortest route could help delivery companies save fuel and cost. Finding the perfect mix of investments from thousands of options could help investment brokers maximise theirs shareholders' value.

## Challenges

However, the journey isn't as easy as outlined here. Today's quantum computers face big hurdles. The building blocks, qubits, are fragile and we don't have enough of them yet. The slightest mechanical vibration or temperature change in the environment can cause them to "pop" and lose their quantum information, which physicists call **decoherence**. Training the algorithms can also be incredibly difficult, and it's still hard to prove when they are truly better than our best classical computers.

## Conclusion

Quantum optimization blends physics and computing. As technology matures, it might unlock solutions classical methods can't reach. Quantum optimization doesn't just accelerate known methods it uses new physical phenomena like [_tunneling_](/en/posts/entries/highschool/qtunnelling) and [_entanglement_](/en/posts/entries/highschool/entanglement) to redefine problem solving strategies. The full potential, particularly for large-scale problems, remains an open question driving current research.
