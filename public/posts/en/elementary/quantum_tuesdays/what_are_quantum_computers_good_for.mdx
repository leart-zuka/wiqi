...
What it actually means to `be able to solve a problem in polynomial time` requires a lot of math and can be ommited here, since we just want to be able to understand what kind of problems we're dealing with.

If you want to get a better insight though, check out the more in-depth version of this [blog post](https://wiqi.info/en/posts/quantum_tuesdays/highschool/what_are_quantum_computers_good_for).
...
- Search algorithms that find the shortest path on a map e.g. the [Dijkstra algorithm](https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm)
...
NP-problems are defined by the fact that they are very hard to solve, but if we were to be given a solution, we can easily and `efficiently` (here meaning in polynomial time, again if you want to read more about this check out the post [here](https://wiqi.info/en/posts/quantum_tuesdays/highschool/what_are_quantum_computers_good_for)) verify if that solution is correct.
