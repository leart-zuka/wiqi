...
A qubit is unfortunately not as easy to understand, and requires a lot of math to get the most correct understanding.
...
I'm sure most of us were in this situation or have kids that we have put in this exact situation.
...
But the interesting features lie in the fact that the qubit randomly decides which value, so either "1" or "0" or commonly represented as $$\ket{1}$$ or $$\ket{0}$$ it wants to hold when performing the actual measurement.
...
The most notable 2 qubit logical gate is the so called `C-NOT` or `controlled NOT` gate. This gate allows us to perform a rotation on one qubit (typically around the x-axis) based on the state of the other qubit. The one qubit we use to check if we're going to perform the rotation is called the `controlling qubit` (indicated in the images by being on the side with the dot) and the other qubit is called the `target qubit` (indicated in the images by being on the side with the cross).

So let's say we input two qubits into our C-NOT gate, where both are in a state $$\ket{0}$$ ("arrow pointing up"). Here the first qubit is the `controlling qubit`, but since it's in a state of $$\ket{0}$$, it won't perform a rotation of the `target qubit`.

![CNOT gate 0](/posts/assets/quantum_tuesdays/Bits_to_qubits/c_not_rot_0.png)

If the `controlling qubit` is in a state $$\ket{1}$$ though ("arrow pointing down"), then it will perform a rotation on the `target qubit`.
...
As mentioned before if our qubit state is pointing along the equator, which we can achieve with our `Hadamard Gate`, this then means that the chance of getting either a $$\ket{0}$$ or a $$\ket{1}$$ when performing a measurement are 50/50. This begs the question, but what if we have a qubit in such a state, and then perform a `C-NOT` operation with it as the `controlling qubit` on a different `target qubit`.

Doing so would create something physicists call [Quantum Entanglement](https://wiqi.info/en/posts/entries/elementary/entanglement), as after performing such operations, we would create a state where the measurement of one qubit will directly influence the outcome of the next one, technically allowing for instant communication between two people if both were to share one of the two qubits.
